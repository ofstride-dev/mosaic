...
function ResultScreen({ result, textureSeeds, onBack, onNew, copiedSwatch, copySwatch, copyMoodboardData, openRefine, reshuffling, handleReshuffleImagery, setShareOpen, locks, toggleLock, recolourSwatch, recolouring, onPreview }) {
  ...
  <section style={styles.section}><div style={styles.sectionHeadRow}><h2 style={styles.sectionHeading}>Imagery direction</h2><button type="button" className="mosaic-focusable" style={styles.reshuffleBtn} onClick={handleReshuffleImagery} disabled={reshuffling}><ShuffleIcon /> {reshuffling ? "Reshuffling…" : "Reshuffle imagery"}</button></div><div className="mosaic-imagery-grid">{imageItems.slice(0, 4).map((img, index) => <ImgTile key={index} img={img} area={index === 0 ? "hero" : String.fromCharCode(96 + index)} />)}</div><div className="mosaic-support-row">{imageItems.slice(4, 6).map((img, index) => <ImgTile key={index + 4} img={img} tall />)}</div><p style={styles.imageryLabel}>{imageryLabel}</p></section>
  <section style={styles.section}><h2 style={styles.sectionHeading}>Colour palette</h2><div className="mosaic-palette-grid">{palette.map((c) => <button type="button" key={c.role || c.name} className="mosaic-swatch mosaic-focusable" onClick={() => copySwatch(c.hex)} style={{ ...styles.swatch, background: c.hex, color: getLuminance(c.hex) < .5 ? "#F4F0E8" : "#252A28" }}><span style={styles.swatchTop}><span style={styles.swatchName}>{c.role || c.name}</span>{copiedSwatch === c.hex ? <small>Copied!</small> : null}</span><span style={styles.swatchHex}>{c.hex.toUpperCase()}</span></button>)}</div></section>
  <section style={styles.section}><h2 style={styles.sectionHeading}>Type with warmth and clarity</h2><div className="mosaic-type-grid"><button type="button" className="mosaic-type-card-copy" onClick={() => copySwatch(headingFont)} style={styles.typeCard}><p style={styles.typeFontLabel}>Heading: {headingFont} · {copiedSwatch === headingFont ? "Copied!" : "Copy"}</p><p style={{ ...styles.typeSpecimenSerif, fontFamily: `'${headingFont}', serif` }}>{headingSpecimen}</p><p style={styles.typeDescriptor}>Sets the tone for this direction</p></button><button type="button" className="mosaic-type-card-copy" onClick={() => copySwatch(bodyFont)} style={styles.typeCard}><p style={styles.typeFontLabel}>Body: {bodyFont} · {copiedSwatch === bodyFont ? "Copied!" : "Copy"}</p><p style={{ ...styles.typeSpecimenSans, fontFamily: `'${bodyFont}', sans-serif` }}>{bodySpecimen}</p><p style={styles.typeDescriptor}>Clear, modern, highly readable</p></button></div></section>
...
const styles = {
  ...
  typeCardHead: { display: "flex", alignItems: "center", justifyContent: "space-between" },
